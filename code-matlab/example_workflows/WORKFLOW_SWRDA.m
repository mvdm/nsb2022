...
photo_cscF.data = locdetrend(photo_cscF.data, 2000, [5 1])';
%photo_cscF.data = locdetrend(photo_cscF.data, 2000)';
%photo_cscF = zscore_tsd(photo_cscF);

%% coherence?
cscF = FilterLFP(cfg, csc);
cscF.data = interp1(cscF.tvec, cscF.data, photo_cscF.tvec);
cscF.tvec = photo_cscF.tvec;

nS = 2048;
[Cxy, F] = mscohere(cscF.data, photo_cscF.data, hann(nS), nS ./ 2, 2^12, 2000);
